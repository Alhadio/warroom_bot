...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        'بناء الصفقة - Stop Loss واهداف محسوبة\n'
...
async def cmd_macro(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if blocked:      verdict = 'لا تداول اليوم - شرط الزامي مكسور'
    elif score >= 4: verdict = 'بيئة ممتازة - تداول بثقة'
    elif score >= 3: verdict = 'بيئة جيدة - تداول بحذر'
    else:            verdict = 'بيئة ضعيفة - تجنب'
...
async def stock_sym(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not q:
        await update.message.reply_text(f'لم يتم العثور على {sym}')
        return STOCK_SYM
...
async def pa_sym_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    q, rsi, macd, ma50, ma200, atr, bb = await asyncio.gather(
        fetch_quote(sym), fetch_rsi(sym), fetch_macd(sym),
        fetch_sma(sym, 50), fetch_sma(sym, 200), fetch_atr(sym), fetch_bbands(sym),
    )
    auto = {}
    summary = []
    if q:
    ...
    if rsi:
        auto['rsi'] = rsi
        summary.append(f'RSI: {rsi:.1f} - {"فوق 30 جيد" if rsi > 30 else "تحت 30 - انعكاس محتمل"}')
    if macd:
        auto['macd'] = macd
        summary.append(f'MACD: {"فوق Signal - ايجابي" if macd["macd"] > macd["signal"] else "تحت Signal"}')
    if ma50:
        auto['ma50'] = ma50
...
async def _finish_pa(msg, ctx):
    ...
    await msg.reply_text(
        ...
        f'التقييم: {g}\n{v}\nحجم الصفقة: {sz}{price_txt}{atr_txt}',
        reply_markup=InlineKeyboardMarkup([
...
async def pb_sym_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    q, rsi, macd, ma20, ma50, ma200, atr = await asyncio.gather(
        fetch_quote(sym), fetch_rsi(sym), fetch_macd(sym),
        fetch_sma(sym, 20), fetch_sma(sym, 50), fetch_sma(sym, 200), fetch_atr(sym),
    )
    auto = {}
    summary = []
    if q:
...
async def _ask_pb(msg, ctx):
    ...
    if 'MA200' in q and auto.get('ma200') and auto.get('price'):
        ...
        m = auto['macd']
        hint = f'\nMACD: {m["macd"]:.3f} - {"فوق الصفر" if m["macd"]>0 else "تحت الصفر"}'
...
async def auto_trade_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text(f'جاري جلب بيانات {sym}...')
    q, atr, ma50 = await asyncio.gather(
        fetch_quote(sym), fetch_atr(sym), fetch_sma(sym, 50),
    )
    if not q:
...
async def trade_plan_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if price:
        await update.callback_query.message.reply_text(
            f'السعر الحالي: ${price:.2f}\n\nادخل سعر الدخول او اضغط استخدام السعر الحالي',
            reply_markup=InlineKeyboardMarkup([
...
async def trade_entry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f'سعر الدخول: ${entry:.2f}\n\nادخل Stop Loss\nمثال: {entry*0.97:.2f}')
        return TRADE_STOP
    except:
...
async def journal_followed_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text('الدرس المستفاد\nاكتب او ارسل - للتخطي')
